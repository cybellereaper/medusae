...
import com.github.cybellereaper.client.CommandRegistration;
...
void registerCommands(DiscordClient client, String guildId) {
    List<CommandRegistration> commands = List.of(
            createPingCommand(client),
            createEchoCommand(client),
            createTypedEchoCommand(client),
            createInspectUserCommand(client),
            createQuoteMessageCommand(client)
    );

    if (guildId == null || guildId.isBlank()) {
        client.registerAndSyncGlobalCommands(commands);
    } else {
        client.registerAndSyncGuildCommands(guildId, commands);
    }
}

CommandRegistration createPingCommand(DiscordClient client) {
    SlashCommandDefinition definition = SlashCommandDefinition.simple("ping", "Reply with pong")
    ...
    return CommandRegistration.builder(definition)
            .onSlash(interaction -> {
...
CommandRegistration createEchoCommand(DiscordClient client) {
...
CommandRegistration createTypedEchoCommand(DiscordClient client) {
    SlashCommandDefinition definition = new SlashCommandDefinition(
            "echo_typed",
...
CommandRegistration createInspectUserCommand(DiscordClient client) {
    return CommandRegistration.builder(SlashCommandDefinition.userContextMenu("Inspect User"))
            .onUserContextMenu(interaction -> client.respondEphemeral(interaction, "User inspection invoked."))
            .build();
}

CommandRegistration createQuoteMessageCommand(DiscordClient client) {
    return CommandRegistration.builder(SlashCommandDefinition.messageContextMenu("Quote Message"))
            .onMessageContextMenu(interaction -> client.respondEphemeral(interaction, "Message quote command invoked."))
            .build();
}
...
void registerInteractions(DiscordClient client) {
